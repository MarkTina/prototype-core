...
- 内核与移动端/PC 最小示例均可构建，现有自动化测试为 36 项。
...
- Bug 归属支持多选并至少保留一项，选项包含 UI 设计、前端开发和 python/桌面开发；发生侧可区分客户端、后端、硬件设计、嵌入式、web 端、桌面端和小程序端。新数据使用 `ownerRoles` 保存完整归属，同时保留首项 `ownerRole` 供旧客户端读取，旧单归属数据和旧提交草稿会在读取时转换为单元素数组。
- Bug 图片上传的 OSS 对象路径包含每张待上传图片的唯一 ID；同一秒上传同名图片或清洗后同名的中文图片时不会再互相覆盖。
- Bug 提报人、操作人与协作者昵称共用同源浏览器的当前使用者记忆，兼容旧姓名缓存；手动输入非空姓名即保存，取消或提交失败不清理。修复人独立记忆，优先使用工单已有值，否则使用上次手填值；自动回填与草稿恢复不会反向改写默认姓名，存储不可用时仅保留会话内记忆。
- Bug 管理页可直接读取 Gitee 最新整文件并导出 Excel；导出不受筛选和分页影响，分别生成 Bug 明细、附件和状态历史三张表。附件保留原图链接并尽量嵌入缩略图，远端不可用或数据不完整时不使用缓存冒充最新结果。
- `#/test-cases` 提供跨移动端和 PC 页面的测试用例工作台；固定 7 个业务字段，支持单条显式保存、Gitee 冲突保护、未知页面保留，以及完整 JSON/Excel 导出。ExcelJS 仅在导出时懒加载，并在内核库构建中 external，避免消费者生产构建二次转换其模块映射。
...
- **决策**: 缓存升级为 schema v3；页面描述与注释按 scope 缓存，流程与 Bug 按整文件缓存，所有远端写入以精确回读结果确认；Bug 编号必须在最新远端数组的变换中分配，SHA 冲突时保留本地提交草稿。
- **原因**: 缓存只能承担快速展示和本地暂存，不能形成独立于 Gitee 的第二真值；客户端预分配连续编号会在多人同时提报时产生重复风险。
- **影响**: AI 修改页面描述 JSON 后必须显式同步 Gitee；人工可对比三类来源并将当前 scope 的缓存或 JSON 显式推送，待推送缓存受保护，远端失败时保留本地内容并允许重试；Bug 提交冲突后重新读取远端，再用原草稿重试且不重复上传已成功附件。
- **日期**: 2026-07-03。
...
- **影响**: 新增 `testCases` 公共协作资源；ExcelJS 仅在导出时懒加载，内核发布包保留裸依赖供消费者单次分包；删除页面或状态不会自动删除历史用例。
- **日期**: 2026-07-15。

### DR-20260810-01: 内核工具文案固定为中文

- **背景**: 内核注释、页面描述和测试工具曾与消费者 `product.copy.zh/en` 共用翻译入口，消费者未配置内核 key 时会直接显示内部标识符。
- **决策**: 内核工具统一读取强类型单一中文文案源；语言切换只作用于消费者业务 UI 和业务状态标签。
- **原因**: 内核工具文案属于公共包职责，不应由每个消费者重复配置，也不应受消费者语言状态影响。
...
- **决策**: 正式发布只允许使用 `publish.yml`；版本提交与标签原子推送，npm 与 Release 幂等发布，失败后通过同一管线恢复当前版本；workflow 修复提交只可在版本尚未发布且旧标签指向其祖先时，用旧 SHA 租约保护移动标签。
...
trigger-reason: Bug 发生侧新增桌面端和小程序端，归属新增 python/桌面开发
updated-by: handoff-maintainer
next-review: 当 Bug 选项或数据兼容规则变化时
-->
