...
- Bug 归属支持多选并至少保留一项，选项包含 UI 设计；发生侧可区分客户端、后端、硬件设计和嵌入式。新数据使用 `ownerRoles` 保存完整归属，同时保留首项 `ownerRole` 供旧客户端读取，旧单归属数据和旧提交草稿会在读取时转换为单元素数组。
...
trigger-reason: 修复 Bug 人员姓名记忆与跨工具同步
updated-by: handoff-maintainer
next-review: 当人员姓名记忆、草稿恢复或协作者身份规则变化时
-->
