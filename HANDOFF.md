...
- 内核与移动端/PC 最小示例均可构建，现有自动化测试为 25 项。
...
- npm 公共发布已可用；当前 latest 为 `@marktowin/prototype-core@1.2.11`。Trusted Publisher 必须与 `MarkTina/prototype-core` 和 `publish.yml` 精确绑定；本地 Token 发布需显式关闭 provenance。
