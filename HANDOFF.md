...
- Bug 归属支持多选并至少保留一项；新数据使用 `ownerRoles` 保存完整归属，同时保留首项 `ownerRole` 供旧客户端读取，旧单归属数据和旧提交草稿会在读取时转换为单元素数组。
...
trigger-reason: Bug 归属升级为兼容旧数据的多选字段
updated-by: handoff-maintainer
next-review: 当 Bug 数据结构或协作写入契约变化时
-->
