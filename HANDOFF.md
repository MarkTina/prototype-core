...
- Bug 归属支持多选并至少保留一项，选项包含 UI 设计和前端开发；发生侧可区分客户端、后端、硬件设计、嵌入式和 web 端。新数据使用 `ownerRoles` 保存完整归属，同时保留首项 `ownerRole` 供旧客户端读取，旧单归属数据和旧提交草稿会在读取时转换为单元素数组。
...
trigger-reason: Bug 发生侧新增 web 端，归属新增前端开发
